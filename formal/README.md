...
| `Formal/Score/Core.lean` | Lean 4 **domain-neutral core** — the three domains, morphisms, stratification, `CouplingWeightVector`, HOA, intervention classes, region geometry. Every peer module imports it. |
| `Formal/Score/CouplingKernel.lean` | The coupling graph's edge rule as a **kernel**: `spectralNorm` (`‖T_κ‖`) as the criticality statistic replacing mean density, subadditivity across the five coupling dimensions with its tightness witness, and the Bollobás–Janson–Riordan phase transition as a declared axiom (`bjr_phase_transition`) so `#print axioms` separates proved from imported. See `obsidian/SCORE/emergence/mechanism/CouplingKernel.md`. |
| `Formal/Score/{Sigma,HOAMaintenance,InscriptionMarket,RevisionLoop,SelfStabilization}.lean` | Core-layer mechanism modules — Σ-actor architecture, the HOA maintenance family, the inscription market, the revision loop, and the abstract self-stabilization predicate. |
| `Formal/Score/{Polaris,Ethos,Agora,Atlas,Nexus,Bac}.lean` | The six **peer implementations**, each refining the core. |
| `Formal/Score.lean` | Aggregator — imports every `Formal/Score/*` module. Add new modules here. |
| `Formal.lean`, `Formal/Basic.lean`, `lakefile.toml`, `lean-toolchain` | Lean project scaffolding (Lean 4 v4.29.1, Mathlib v4.29.1). |
...
| `alloy/score_core.als` (+ `README.md`) | Alloy 6 **bounded model finding** over Core's spine: 16 commands, each with an `expect`; run with `protocol-formal-template/alloy/run-alloy.ps1 -Model formal/alloy/score_core.als`. An exploratory lens, not a proof. |
| `z/*.zed` (+ `README.md`) | Z specifications of the morphisms and HOA maintenance, typechecked by CZT via `make z` (`scripts/z_run.py`). See `z/README.md`. |
| `sketches/SCORE_B3_Context.lean` | A standalone **structural sketch** (Lean 4 core, no Mathlib) from the 2026-09-24 Kant/Hume session: inscriptions as context + content, grounding as a least fixed point, in-force as a greatest. **Not part of the lake build** and not imported by `Formal/Score.lean`. See `../governance/aperture/kant-hume-notes.md` section 12. |
| `prism/*.{sm,csl,nm,pctl}` | PRISM **probabilistic** model-checked dynamics — the *quantitative* rung (probability, expected time, optimal policy) the TLC/SPIN pilots do not reach. `HOADissolution.sm` (CTMC lift of `tla/HOA.tla` §HM8, validating the SEWI critical-slowing-down premise) with its bistability refinement `HOANucleation.sm` (a nucleation barrier giving a stable unformed equilibrium — `HOA.tla` case 3 on a generative chain), `InterventionMDP.nm` (intervention-policy MDP on top of it, → Segment 7, deriving the [[InterventionClasses]] sequencing/sponsor/leverage claims from the optimal policy) with its subtractive-family extension `HOASubtractive.nm` (dissolution cost indexed by target persistence, not operator depth — the VC-1 finding) and its B₃-mediated-channel extension `MediatedChannel.sm` (sub-threshold steering is non-contestable and cohort-forming — the [[IncorporationAsymmetry]] self-masking structure, completing the intervention 2×2), and `AgentLearning.sm` (Path-A/B learning DTMC, → Segment 1, confirming the multigenerational attractor and κ-decay asymmetry against the Brock 2026 baseline) with its multi-agent step `MabmPercolation.sm` (Φ crystallization as a discontinuous percolation cliff — the population-emergent object toward the full MABM), `EthosCapture.sm` (the first **peer** model — ETHOS epistemic capture + critical slowing down, reusing model ①), `AgoraCorrection.sm` (the second peer model — AGORA institutional-correction reachability, reusing model ②), `AtlasCascade.sm` (the third — ATLAS signaling-cascade percolation, reusing model ③), each with a depth tie to its `src/` generator's test suite, and `NexusInnovation.sm` (the fourth, breadth-only — NEXUS innovation-HOA vs NS-cartel tipping, reusing model ①). All carry the pre-registration-lock governance caveat. See `prism/README.md`. |
