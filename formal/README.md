...
| `prism/*.{sm,csl,nm,pctl}` | PRISM **probabilistic** model-checked dynamics — the *quantitative* rung (probability, expected time, optimal policy) the TLC/SPIN pilots do not reach. Three models: `HOADissolution.sm` (CTMC lift of `tla/HOA.tla` §HM8, validating the SEWI critical-slowing-down premise), `InterventionMDP.nm` (intervention-policy MDP on top of it, → Segment 7, deriving the [[InterventionClasses]] sequencing/sponsor/leverage claims from the optimal policy), and `AgentLearning.sm` (Path-A/B learning DTMC, → Segment 1, confirming the multigenerational attractor and κ-decay asymmetry against the Brock 2026 baseline). All carry the pre-registration-lock governance caveat. See `prism/README.md`. |
