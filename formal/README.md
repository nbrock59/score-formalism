...
| `prism/*.{sm,csl,nm,pctl}` | PRISM **probabilistic** model-checked dynamics — the *quantitative* rung (probability, expected time, optimal policy) the TLC/SPIN pilots do not reach. `HOADissolution.sm` is the CTMC lift of `tla/HOA.tla` (§HM8) validating the SEWI critical-slowing-down premise; `InterventionMDP.nm` is the intervention-policy MDP on top of it (→ Segment 7) that derives the [[InterventionClasses]] sequencing / sponsor / leverage claims from the optimal policy. Both carry the pre-registration-lock governance caveat. See `prism/README.md`. |
